...
class AllCoinsMetadata:
    ...
    """
    uniswap_coins = [Dict[str,CoinMetadata]]
    not_uniswap = [Dict[str,CoinMetadata]]
    fail_to_download = [Dict[str,CoinMetadata]]
    ...
    def __post_init__(self):
        ...
        self.all_coins_date_pathdir = data_path / "raw" / "gecko" / self.date_string
...
@dataclass
class AllCoinsMetadata2:
    """
    Manages the download of coin metadata and tracks which coins are downloaded or failed.

    Handles:
    1. Downloaded coins
    2. Coins that failed to download
    3. Time series data for coins
    """
    downloaded_coins: Dict[str, Optional['CoinMetadata']] = field(default_factory=dict)  # Dictionary of downloaded coins {coin_id: CoinMetadata}
    ...
    num_total_downloads: int = 0
    redo_names: bool = False  # For deprecated price change objects

    def __post_init__(self):
        """
        Initialize directories and load existing data if available.
        """
        from spflows import data_path
    ...
    def _load_pickle(self, filepath, default):
        if filepath.exists():
    ...
    def save_lists(self):
        """
        Save downloaded and failed lists to disk.
        """
    ...
    def download_coins_metadata(self, coins_to_download=None, trials=10000):
        """
        Download metadata for all coins provided, regardless of type.
        Args:
            coins_to_download (list): List of coin IDs to download.
            trials (int): Maximum number of download attempts.
        """
        if coins_to_download is None:
    ...
    def download_df_timeseries(self):
        """
        Download time series for all coins and store them in DataFrame format.
        """
        rate_limiter = RateLimitedRequester()
...
if __name__=="__main__":
    ...
    number_of_coins_to_download = 35
